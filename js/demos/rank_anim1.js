...
function animate() {
    ...
    orbs.forEach((orb,i) => {
        ...
        const final_position = new THREE.Vector3(initial_position.x, initial_position.y, spacing*2);

        orb.position.set(...initial_position);

        if(id.z == 0) {
            orb.position.set(...delayed_lerp(id.z/rows, time, initial_position, final_position));
        } else if(llerp(id.z/rows, time, 0, 1) > 0){
        ...
        for(var i = 0; i < last; i++){
            tmp_array.push(array[id.x][id.y][i]);
        }
        
        var delayed_sum = stepwise_sum(tmp_array);
        if(id.z == 0) orb.scale.set(...new THREE.Vector3(delayed_sum(time*3)/7, delayed_sum(time*3)/7, delayed_sum(time*3)/7));
