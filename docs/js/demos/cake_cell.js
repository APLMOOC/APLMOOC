...
// n-cell highlighting control
// 0: individual cell, 1: line (same z,y), 2: layer (same z), 3: whole cake
window.nCell = 0;
...
const positionToMesh = new Map(); // New: reverse mapping for n-cell highlighting
...
function initializeCakeArray(root) {
    ...
    positionToMesh.clear(); // Clear reverse mapping too
            ...
            if (match) {
                ...
                if (x >= 0 && x < 3 && y >= 0 && y < 3 && z >= 0 && z < 4) {
                    ...
                    if (cakeArray[z][x][y] !== null) {
                        ...
                        positionToMesh.set(`${z}-${x}-${y}`, child); // Add reverse mapping
                    }
                } else {
...
function updateAPLDisplay() {
    ...
    // Display N-cell information based on current nCell value
    let nCellText = '';
    switch (window.nCell) {
        case 0:
            nCellText = '0-cells (Individual elements)';
            break;
        case 1:
            nCellText = '1-cells (Lines)';
            break;
        case 2:
            nCellText = '2-cells (Layers)';
            break;
        case 3:
            nCellText = '3-cells (Whole array)';
            break;
        default:
            nCellText = '0-cells (Individual elements)';
            break;
    ...
    display += `<div style="color: #FFFF66; font-weight: bold; margin-bottom: 15px; padding: 5px; background-color: rgba(0,0,0,0.3); border-radius: 5px; display: inline-block;">${nCellText}</div>`;
...
let lastHighlightedMeshes = []; // Track all highlighted meshes for restoration

// Function to highlight n-cells based on the window.nCell value
function highlightNCell(baseX, baseY, baseZ) {
    ...
    // Restore all previously highlighted meshes
    lastHighlightedMeshes.forEach(mesh => restoreMesh(mesh));
    lastHighlightedMeshes = [];
    
    // Apply highlighting based on nCell value
    switch (window.nCell) {
        case 0: // Individual cell (0-cell)
            if (cakeArray[baseZ][baseX][baseY]) {
                cakeArray[baseZ][baseX][baseY].highlighted = true;
                const mesh = positionToMesh.get(`${baseZ}-${baseX}-${baseY}`);
                if (mesh) {
                    highlightMesh(mesh);
                    lastHighlightedMeshes.push(mesh);
                }
            }
            break;
            
        case 1: // Line of cells with same z and y (1-cell)
            for (let x = 0; x < 3; x++) {
                if (cakeArray[baseZ][x][baseY]) {
                    cakeArray[baseZ][x][baseY].highlighted = true;
                    const mesh = positionToMesh.get(`${baseZ}-${x}-${baseY}`);
                    if (mesh) {
                        highlightMesh(mesh);
                        lastHighlightedMeshes.push(mesh);
                    }
        ...
        case 2: // Layer with same z (2-cell)
            for (let x = 0; x < 3; x++) {
                for (let y = 0; y < 3; y++) {
                    if (cakeArray[baseZ][x][y]) {
                        cakeArray[baseZ][x][y].highlighted = true;
                        const mesh = positionToMesh.get(`${baseZ}-${x}-${y}`);
                        if (mesh) {
                            highlightMesh(mesh);
                            lastHighlightedMeshes.push(mesh);
                        }
        ...
        case 3: // Whole cake (3-cell)
            for (let z = 0; z < 4; z++) {
                for (let x = 0; x < 3; x++) {
                    for (let y = 0; y < 3; y++) {
                        if (cakeArray[z][x][y]) {
                            cakeArray[z][x][y].highlighted = true;
                            const mesh = positionToMesh.get(`${z}-${x}-${y}`);
                            if (mesh) {
                                highlightMesh(mesh);
                                lastHighlightedMeshes.push(mesh);
                            }
        ...
        default:
            // Default to individual cell for invalid values
            if (cakeArray[baseZ][baseX][baseY]) {
                cakeArray[baseZ][baseX][baseY].highlighted = true;
                const mesh = positionToMesh.get(`${baseZ}-${baseX}-${baseY}`);
                if (mesh) {
...
function onPointerMove(event) {
    ...
    if (intersects.length > 0) {
        ...
        if (mesh !== lastHovered) {
            if (lastHovered) {
                // Don't restore here since highlightNCell handles mesh restoration
            }
            
            lastHovered = mesh;
            // Don't highlight the single mesh here - highlightNCell handles all highlighting
            
            const currentPos = meshToPosition.get(mesh);
            
            if (currentPos) {
                // Use the n-cell highlighting function
                highlightNCell(currentPos.x, currentPos.y, currentPos.z);
            } else {
        ...
        if (lastHovered) {
            // Restore all highlighted meshes
            lastHighlightedMeshes.forEach(mesh => restoreMesh(mesh));
            lastHighlightedMeshes = [];
